...
async function loadCatalog(force = false) {
    ...
    const data = await fetchJson(`${githubContentUrl()}?ref=${encodeURIComponent(branch)}`, { headers: githubHeaders() });
...
function renderBatch() {
    ...
    state.batch.forEach((item, idx) => {
        ...
        tr.innerHTML = `
            ...
            <td><div class="batch-yt-title">${esc(item.ytTitle || item.link)}</div><div class="batch-vid">${esc(item.videoId)}${item.catalogId && item.status === 'added' ? ' · ' + esc(item.catalogId) : ''}</div></td>
            <td>${best}</td>
            <td>${conf}</td>
            <td>
